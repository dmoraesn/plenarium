{{-- Este formulário agora contém a tag <form> e a lógica de criar/editar --}}
<form method="POST" action="{{ $model->exists ? route('config.partidos.update', $model) : route('config.partidos.store') }}">
    @csrf
    @if ($model->exists)
        @method('PUT')
    @endif
    ...
    <div class="mt-6 flex justify-end gap-3">
        {{-- ROTA CORRIGIDA --}}
        <a href="{{ route('config.partidos.index') }}" class="px-3 py-2 rounded border border-gray-300 text-gray-700">Cancelar</a>
        <button class="px-3 py-2 rounded bg-indigo-600 text-white hover:bg-indigo-700">
            {{ $model->exists ? 'Atualizar' : 'Salvar' }}
        </button>
    </div>
</form>