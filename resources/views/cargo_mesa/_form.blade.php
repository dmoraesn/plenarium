...
<div class="grid grid-cols-1 md:grid-cols-3 gap-4">
    ...
    <div class="md:col-span-3 flex items-center gap-6">
        <label class="inline-flex items-center">
            <input type="checkbox" name="cargo_unico" value="1" class="rounded border-gray-300" {{ old('cargo_unico', $model->cargo_unico ?? false) ? 'checked' : '' }}>
            <span class="ml-2 text-sm text-gray-700">Cargo único</span>
        </label>
        <label class="inline-flex items-center">
            <input type="checkbox" name="ativo" value="1" class="rounded border-gray-300" {{ old('ativo', $model->ativo ?? true) ? 'checked' : '' }}>
...
<div class="mt-6 flex justify-end gap-3">
    <a href="{{ route('config.cargos-mesa.index') }}" class="px-3 py-2 rounded border border-gray-300 text-gray-700">Cancelar</a>
