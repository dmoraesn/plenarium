...
<div class="max-w-4xl mx-auto py-8 px-4 sm:px-6 lg:px-8">
    <div class="flex items-center justify-between mb-6">
        <div>
            <h1 class="text-2xl font-bold text-gray-900">Editar Sessão {{ $sessao->numero }}/{{ $sessao->ano }}</h1>
            <p class="text-sm text-gray-500">Ajuste os detalhes da sessão legislativa.</p>
        </div>
        ...
        <form method="POST" action="{{ route('sessoes.update', $sessao) }}">
            ...
            <div class="p-6 space-y-6">
                ...
                <div>
                    ...
                    <input type="date" name="data" id="data" value="{{ old('data', $sessao->data->format('Y-m-d')) }}" required class="mt-1 block w-full md:w-1/3 rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm @error('data') border-red-500 @enderror">
                    @error('data')
                        <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
